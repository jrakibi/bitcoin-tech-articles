...
const BitcoinTransactionViewer: React.FC<{ detail: any }> = ({ detail }) => {
  const displayAsStack = (asm: string) => {
    ...
    return parts.map((part, index) => (
      <div key={index} className={part.startsWith('OP_') ? 'text-red-500' : 'text-black'}>
        {part}
      </div>
    ))
  }
      ...
      <div className="mb-6 rounded-lg bg-gray-100 p-3">
        <div className="h-full space-y-4 rounded-lg font-mono text-sm">
          <div className="text-center">
            {txTitle && <h1 className="mb-1 text-base font-semibold">Transaction: {txTitle}</h1>}
            {txId && (
            ...
            <div className="flex w-full flex-col p-2 lg:w-1/2">
              {decodedTransaction?.inputs?.map((input, index) => (
                ...
                >
                  ...
                  <div className="text-left text-sm text-gray-600">Click to view details</div>
                </button>
              ))}
            </div>

            {decodedTransaction?.inputs?.length > 0 && decodedTransaction?.outputs?.length > 0 && (
              <svg
            ...
            <div className="flex w-full flex-col p-2 lg:w-1/2">
              {decodedTransaction?.outputs?.map((output, index) => (
                ...
                >
                  ...
                  <div className="text-left text-sm text-gray-600">{output.value} BTC</div>
                </button>
              ))}
            </div>
          </div>
