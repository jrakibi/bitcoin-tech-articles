...
This transaction type is one of the simplest Bitcoin script types to understand, making it an ideal starting point for laying the foundation for more complex ones.
...
To help us understand how P2PK works, we will explain it using the example of the **first transaction in Bitcoin history**.  
It’s a P2PK transaction from [block 170](https://mempool.space/tx/f4184fc596403b9d638783cf57adfe4c75c605f6356fbc91338530e9831e9e16) which includes a payment of 10 BTC from Satoshi Nakamoto to Hal Finney.
...
To see this in action click on the **Output 2** of this Transaction too see the HEX and ASM representation of scriptPubKey
...
- **0411db ... b412a3**: Represents the public key of Hal Finney
- **OP_CHECKSIG**: This opcode, shown as "0xac" at the end of the HEX section above, verifies an ECDSA signature. It returns TRUE if the signature is valid, otherwise FALSE.
...
For our example this is it:: The scriptSig is the signature followed by SigHash byte as shown below
Click on the transaction below to see it in action:
